	...
	$('#smallPanelBigSortSelect').on('change', function() {
		$.ajax({
			...
			data: {
				...
				$('#smallPanelMiddleSortSelect').append("<option value=''> == 중분류 선택 == </option>");
				for (var i = 0; i < result.length; i++) {
					if(bigId == result[i].id){
						var option = $("<option selected value=" + result[i].id + ">" + result[i].name + "</option>");
					}else{
						var option = $("<option value=" + result[i].id + ">" + result[i].name + "</option>");
					}
		...
		$('#smallPanelMiddleSortSelect').on('change', function() {
			$.ajax({
				...
				data: {
					...
					$('#smallPanelSmallSortSelect').append("<option value=''> == 소분류 선택 == </option>");
					for (var i = 0; i < result.length; i++) {
						var option = $("<option value=" + result[i].id + ">" + result[i].name + "</option>");
...
function changeModal(id){
	$('input[name=productIdInput]').val(id);
}

function changeSingleModal(id, code){
	$('input[name=productIdInput]').val(id);
	$('input[name=productCodeInput]').val(code);
}
