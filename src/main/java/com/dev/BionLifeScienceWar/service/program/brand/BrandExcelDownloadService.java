...
public class BrandExcelDownloadService {
	...
	public void bigSortDownload(
		...
		Sheet brand = workbook.createSheet("브랜드");
		...
		brand.setDefaultColumnWidth(40);
		...
		String brandHeaderNames[] = new String[] {
				"브랜드 ID", "브랜드 이름"
		};
		...
		Sheet[] sheets = {brand, bigSort, middleSort, smallSort, product, productInfo, productSpec};
		...
		List<BrandProductInfo> productInfos = productInfoRepository.findAll();
		List<BrandProductSpec> productSpecs = productSpecRepository.findAll();

		/**
		 * body data
		 */
		String brandBodyDatass[][] = new String[brands.size()][headers.get(0).length];
		String bigBodyDatass[][] = new String[bigSorts.size()][headers.get(1).length];
		String middleBodyDatass[][] = new String[middleSorts.size()][headers.get(2).length];
		String smallBodyDatass[][] = new String[smallSorts.size()][headers.get(3).length];
		String productBodyDatass[][] = new String[products.size()][headers.get(4).length];
		String productInfoBodyDatass[][] = new String[productInfos.size()][headers.get(5).length];
		String productSpecBodyDatass[][] = new String[productSpecs.size()][headers.get(6).length];
		...
		for(int x=0; x<brandBodyDatass.length; x++) {
			brandBodyRow = brand.createRow(brandRowCount++);
			for(int i=0; i<brandBodyDatass[x].length; i++) {
				brandBodyCell = brandBodyRow.createCell(cellNumbber);
				brandBodyCell.setCellValue(brands.get(x).getId());
				brandBodyCell.setCellStyle(bodyXssfCellStyle);
				
				brandBodyCell = brandBodyRow.createCell(cellNumbber+1);
				brandBodyCell.setCellValue(brands.get(x).getName());
				brandBodyCell.setCellStyle(bodyXssfCellStyle);
			}
		}
		...
		for (int x=0; x<productBodyDatass.length; x++) {
			...
			for (int i = 0; i < productBodyDatass[x].length; i++) {
				...
				productBodyCell.setCellValue(products.get(x).getBrandProductCode()); 
				...
				productBodyCell.setCellValue(products.get(x).getBrand().getId());// 데이터 추가
		 ...
		 * download
		 */
		String fileName = "BRAND_PRODUCT_RESET_SHEET";
	...
	public void productAddSheetDownload(
		...
		int brandRowCount = 1; // 데이터가 저장될 행
		int bigRowCount = 1; // 데이터가 저장될 행
		int middleRowCount = 1;
		int smallRowCount = 1;
		 ...
		 * download
		 */
		String fileName = "BRAND_PRODUCT_ADD_SHEET";
